...
* **Richtige OOP-Umsetzung (Erforderlich):** Statische Methoden sind **nur erlaubt**, wenn sie als **"Prestructors" (benannte Konstruktoren)** dienen. Sie müssen **sofort** den **einzigen primären Konstruktor** (siehe 1.5) aufrufen, um das Objekt zu erzeugen. Sie dürfen **keine** I/O- oder Business-Logik ausführen, sondern nur Daten vorbereiten (z.B. Typkonvertierung) oder eine Default-Implementierung instanziieren. *Beispiel:* User.withNameAndEmail("John", "j@e.com").
...
public class TaskFolder implements Folder {  
    ...
    public TaskFolder(String name) { this.name \= name; }

    @Override  
    public void displayTo(Tree treeComponent) {  
        // 1\. Delegation der Darstellung:  
