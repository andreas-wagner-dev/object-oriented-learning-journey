...
Die von Jimmy Bogard populär gemachte Vertical Slice Architecture (VSA) wird in der .NET-Community oft fälschlicherweise als kosmetisches Update der Clean Architecture missverstanden. Doch das ist ein fundamentaler Irrtum: VSA ist ein radikaler Gegenentwurf, der die Kernannahmen schichtenbasierter Architekturen direkt angreift. Bogard begründet dieses Muster mit der Frustration über starre Schichtenregeln und ausuferndes Mocking:

„Das Problem ist, dass dieser Ansatz nur für eine Minderheit der typischen Anforderungen geeignet ist. Zudem sind diese Architekturen extrem mock-lastig und erzwingen starre Regeln beim Abhängigkeitsmanagement. Es entstehen unzählige Abstraktionen um Konzepte, die nicht abstrahiert werden sollten (Controller ➔ Service ➔ Repository). Stattdessen möchte ich jede Anfrage als einen maßgeschneiderten, eigenständigen Anwendungsfall betrachten.“

In seinem Artikel „Why Vertical Slices Won't Evolve from Clean Architecture“ räumt der Software-Architekt Rico Fritzsche mit diesen Missverständnissen auf. Er entlarvt den weit verbreiteten LinkedIn-Hype, bei dem Schichten einfach nur in Feature-Ordner umtopft werden, als „übertechnisiertes Gerümpel, nur neu arrangiert“.

Rico Fritzsche löst dabei das eigentliche Kernproblem der inneren Feature-Architektur: Er bricht das technische Dogma, schmilzt unnötige Abstraktionen und schmeißt MediatR-Zeremonien aus einfachen Slices. Für Standard-Lese- oder Schreibvorgänge schlägt er pragmatisch simple Transaktionsskripte oder direkte SQL-Abfragen vor – was Code schneller, lesbarer und wartbarer macht.

Diesen pragmatischen Ansatz möchte ich in diesem Artikel um eine radikal fachliche Ausrichtung auf der Makro-Ebene ergänzen. Denn während im Inneren der Features die Technik schmilzt, hält die Struktur im Großen oft weiterhin an einer rein technischen Denkweise fest: Auf der obersten Verzeichnisebene (Features/, Infrastruktur/, Shared/) regieren immer noch die alten, technischen Schubladen.

Das Business denkt jedoch ausschließlich in Substantiven (Kunde, Auto, Buchung) und Verben (registrieren, warten, bezahlen) – niemals in technischen Werkzeugen wie Handlern, Controllern oder DTOs. Wenn wir dieses technische Fundament auf der Root-Ebene nicht aufbrechen, verlagern wir die altbekannten Probleme der Clean Architecture lediglich an eine andere Stelle. Trotz strategischer DDD-Konzepte wie der „Allgegenwärtigen Sprache“ und dem Ideal einer Screaming Architecture dominiert die Technik dann weiterhin die Struktur. Es ist Zeit, die technische Brille auch beim Datei-Layout komplett abzusetzen.
