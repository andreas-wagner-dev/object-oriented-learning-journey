...
2. **Event Delivery in exchange/:** The technical messaging infrastructure (e.g., RabbitMQ, Apache Kafka, AWS EventBridge) is isolated as an adapter inside exchange/events/.
...
export class NodeJsMovieApp implements MovieApp {  
  ...
  constructor() {  
    ...
    const rawApi \= new MoviePoolApi(process.env.MOVIE\_API\_URL || 'https://api.movies.com');  
    const authService \= new JwtAuthApi(process.env.JWT\_SECRET || 'super-secret-key');
