...
SRP auf eine **pragmatische und objektive Weise** zu definieren, damit es direkt in unsere tägliche Programmplanung integriert werden kann. Nicht als abstraktes Designideal, das im Code-Review interpretiert wird, sondern als messbare Eigenschaft, die Werkzeuge prüfen und Entwickler gezielt anstreben können.
...
| **Hohe Wahrscheinlichkeit lokaler Änderung** | LCOM4 | = 1 | ❌ Logging-Änderung betrifft alle 3 Methoden des Service | ✅ `AuditingOrderService` isoliert diese Änderung vollständig |
| **Geringe Ausbreitung von Änderungen** | CBO | niedrig | ❌ Änderung an `PaymentGateway` trifft den gesamten `OrderService` | ✅ Nur `DefaultOrderService` ist betroffen |
